import { hasInjectionContext, getCurrentInstance, version, unref, inject, h, toRef, isRef, readonly, defineComponent, provide, shallowReactive, ref, Suspense, nextTick, Transition, useSSRContext, computed, withAsyncContext, createApp, effectScope, reactive, toValue, defineAsyncComponent, onErrorCaptured, onServerPrefetch, createVNode, resolveDynamicComponent, shallowRef, isReadonly, isShallow, isReactive, toRaw, mergeProps } from 'vue';
import { u as useRuntimeConfig$1, $ as $fetch$1, w as withQuery, m as hasProtocol, p as parseURL, n as isScriptProtocol, j as joinURL, c as createError$1, o as defu, q as sanitizeStatusCode, t as createHooks, v as hash, x as appendHeader, y as getRequestHeaders, z as sendRedirect } from '../nitro/node-server.mjs';
...
const _routes = [
  {
    ...
    component: () => import('./_nuxt/index-30defad6.mjs').then((m) => m.default || m)
  }
];
...
const useComediansStore = () => {
  const comedians = useState("comedians", () => []);
  return comedians;
};
const useAllComedian = async () => {
  const comedianStore = useComediansStore();
  try {
    const { data, error } = await useFetch("/api/getAllComedian", "$caMff1eWiA");
    if (error.value) {
      throw createError({
    ...
    comedianStore.value = data.value;
  } catch (error) {
    throw createError({
      statusCode: 500,
      statusMessage: "全ての芸人を取得することができません。"
...
const _sfc_main$2 = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    let __temp, __restore;
    [__temp, __restore] = withAsyncContext(() => useAllComedian()), await __temp, __restore();
...
const _sfc_main$1 = {
  ...
  setup(__props) {
    ...
    const stack = void 0;
    const _Error404 = defineAsyncComponent(() => import('./_nuxt/error-404-e77efab1.mjs').then((r) => r.default || r));
    const _Error = defineAsyncComponent(() => import('./_nuxt/error-500-4ab8c9a0.mjs').then((r) => r.default || r));
...
const _sfc_main = {
  ...
  __ssrInlineRender: true,
  setup(__props) {
    const IslandRenderer = defineAsyncComponent(() => import('./_nuxt/island-renderer-24dc19a9.mjs').then((r) => r.default || r));
    const nuxtApp = /* @__PURE__ */ useNuxtApp();
...
export { LayoutMetaSymbol as L, PageRouteSymbol as P, _wrapIf as _, useRuntimeConfig as a, navigateTo as b, createError as c, useRoute as d, entry$1 as default, appLayoutTransition as e, useNuxtApp as f, useState as g, useComediansStore as h, injectHead as i, useAuth as j, nuxtLinkDefaults as n, resolveUnrefHeadInput as r, useRouter as u };
